...
// from-WinHtut-to-lonelone-100
    ...
    if(who == 't'){
        ...
        for(int i=0; i<25; i++){
            ...
            db[transfer].trc[space_array[transfer]-15].note[index_point]=timeDb[0].time_string[i];
            index_point++;
        }
        ...
        get_time();
        for(int a=0; a<25; a++){
            db[receiver].trc[space_array[receiver]-15].note[index_point]=timeDb[0].time_string[a];
            index_point++;
        }
...
void get_time(){
    time_t current_time_variable;
    time(&current_time_variable);

    printf("Current time is ---> %s\n", ctime(&current_time_variable));

    FILE *fptr = fopen("file1.txt","w");
    fprintf(fptr,"%s", ctime(&current_time_variable));

    fclose(fptr);

    int index=0;
    int time_space_counter=0;
    //int anti_time_space_counter=0;

    timeDb[0].time_string[index]='-';
    index++;
...
// size counter
int phone_number_counter(unsigned long long int toCount){
    int toCountPhoneNumber =0;
     do {
        toCountPhoneNumber++;
        toCount /= 10;
    } while (toCount != 0);

    return toCountPhoneNumber;
}
